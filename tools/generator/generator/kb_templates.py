...
def render_document(doc: dict) -> str:
    ...
    if domain == "it":
        from .it_corpus import agent_excerpt, header, kb_title_for_doc, lookup_kb_body

        if kind in ("system_prompt", "agent_role", "agent_skill"):
            slug = doc.get("agent_slug", "")
            excerpt = agent_excerpt(slug, kind)
            if excerpt:
                label = {"system_prompt": "System prompt", "agent_role": "Role", "agent_skill": "Skill"}[kind]
                return header(doc) + f"## {label}: {doc.get('agent_name', title)}\n\n{excerpt}\n"
        body = lookup_kb_body(doc)
        if body:
            mapped_title = kb_title_for_doc(doc) or title
            doc = {**doc, "title": mapped_title}
