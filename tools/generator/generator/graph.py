...
def load_blueprint(domain: str) -> dict[str, Any]:
    return load_yaml_simple(BLUEPRINTS_ROOT / "domains" / f"{domain}.yaml")
...
def emit_kb_structure(
    state: GraphState, min_docs: dict[str, int], domains: tuple[str, ...] | None = None
) -> None:
...
def emit_agent_kb_structure(state: GraphState, domains: tuple[str, ...] | None = None) -> None:
    active = domains or DOMAINS
...
def wire_agent_relationships(state: GraphState, domains: tuple[str, ...] | None = None) -> None:
    active = domains or DOMAINS
    for domain in active:
...
def build_relationship_graph(min_docs: dict[str, int], domains: tuple[str, ...] | None = None) -> GraphState:
    active = domains or DOMAINS
    state = GraphState()
    ...
    emit_kb_structure(state, min_docs, domains=active)
    emit_agent_kb_structure(state, domains=active)
    wire_agent_relationships(state, domains=active)
    ...
    if len(state.relationships) < 2500:
        ...
        while len(state.relationships) < 2500 and attempts < 5000:
            attempts += 1
            domain = state.rng.choice(list(active))
