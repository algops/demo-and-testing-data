...
2. The Merge Request **status** must be tracked with **Label** `CR State` to reflect in what state the merge request is (see Labels).
...
There is a separate document on how to do Code Reviews.
...
As it is explained in Issue Lifecycle flow, the MR can be merged to `main` and therefore **marked as resolved only when**:
